...
import net.eyone.yonewi_mapping.mapper.EncounterExtensionsMapper;
...
public final class AmbulatoryEncounterService {
    ...
    public static EncounterR4 toR4(Ambulatory in) {
        ...
        try {
            ...
            log.info("[AmbulatoryEncounterMapper] [toR4] out.resourceType : {} , valeur", safeToString(out.resourceType));
            if (in.getData() != null) {
            ...
            meta.lastUpdated = DateUtil.toIsoDateTime(safe(in.getData() != null ? in.getData().getCreationDate() : null));
            log.info("[AmbulatoryEncounterMapper] [toR4] meta.lastUpdated : {} , valeur", safeToString(meta.lastUpdated));
            meta.profile = List.of(MappingConstants.PROFILE_ENCOUNTER_AMB);
            log.info("[AmbulatoryEncounterMapper] [toR4] meta.profile[0] : {} , valeur", MappingConstants.PROFILE_ENCOUNTER_AMB);
            ...
            if (in.getData() != null) {
                addIdentifierR4(out, MappingConstants.SYS_IDENTIFIER_PRESTATION, stringOrNull(in.getData().getIdentifier()));
            ...
            // status
            out.status = mapStatusR4(code(in.getData() != null && in.getData().getStatus() != null ? in.getData().getStatus().getCode() : null));
            ...
            if (in.getData() != null && in.getData().getMedicalActCategory() != null) {
                ...
                coding.system = MappingConstants.SYS_MEDICAL_ACT_CATEGORY; log.info("[AmbulatoryEncounterMapper] [toR4] type[0].coding[0].system : {} , valeur", safeToString(coding.system));
            ...
            if (in.getData() != null && in.getData().getDenomination() != null) {
                ...
                coding.system = MappingConstants.SYS_SERVICE_DENOMINATION; log.info("[AmbulatoryEncounterMapper] [toR4] serviceType.coding.system : {} , valeur", safeToString(coding.system));
                EncounterR4.CodeableConcept cc = new EncounterR4.CodeableConcept();
                cc.coding = List.of(coding);
            ...
            // extensions (R4)
            out.extension = EncounterExtensionsMapper.buildEncounterR4Extensions(in); log.info("[AmbulatoryEncounterMapper] [toR4] out.extension.size : {} , valeur", out.extension == null ? 0 : out.extension.size());
            ...
            if (in.getData() != null && in.getData().getRelatedService() != null) {
                ...
                Long id = in.getData().getRelatedService().getIdentifier(); log.info("[AmbulatoryEncounterMapper] [toR4] relatedService.identifier : {} , valeur", safeToString(id));
                String name = in.getData().getRelatedService().getName(); log.info("[AmbulatoryEncounterMapper] [toR4] relatedService.name : {} , valeur", safeToString(name));
                if (id != null) {
                    locInner.reference = "Location/" + id; log.info("[AmbulatoryEncounterMapper] [toR4] location.reference : {} , valeur", safeToString(locInner.reference));
                    locInner.display = name; log.info("[AmbulatoryEncounterMapper] [toR4] location.display : {} , valeur", safeToString(locInner.display));
                    EncounterR4.EncounterLocation el = new EncounterR4.EncounterLocation();
                    el.location = locInner;
                    if (out.period != null) {
                        el.period = out.period; log.info("[AmbulatoryEncounterMapper] [toR4] location.period(start) : {} , valeur", safeToString(out.period.start));
                    }
                    out.location = List.of(el); log.info("[AmbulatoryEncounterMapper] [toR4] out.location.size : {} , valeur", 1);
                }
            }
    ...
    public static EncounterR5 toR5(Ambulatory in) {
        ...
        try {
            ...
            out.resourceType = MappingConstants.RESOURCE_TYPE_ENCOUNTER; log.info("[AmbulatoryEncounterMapper] [toR5] out.resourceType : {} , valeur", safeToString(out.resourceType));
            if (in.getData() != null) {
            ...
            meta.versionId = MappingConstants.META_VERSION; log.info("[AmbulatoryEncounterMapper] [toR5] meta.versionId : {} , valeur", safeToString(meta.versionId));
            ...
            meta.profile = List.of(MappingConstants.PROFILE_ENCOUNTER_AMB); log.info("[AmbulatoryEncounterMapper] [toR5] meta.profile[0] : {} , valeur", MappingConstants.PROFILE_ENCOUNTER_AMB);
            ...
            if (in.getData() != null) {
                addIdentifierR5(out, MappingConstants.SYS_IDENTIFIER_PRESTATION, stringOrNull(in.getData().getIdentifier()));
            ...
            if (in.getData() != null && in.getData().getMedicalActCategory() != null) {
                ...
                typCoding.system = MappingConstants.SYS_MEDICAL_ACT_CATEGORY; log.info("[AmbulatoryEncounterMapper] [toR5] type[0].coding[0].system : {} , valeur", safeToString(typCoding.system));
            ...
            if (in.getData() != null && in.getData().getDenomination() != null) {
                ...
                svCoding.system = MappingConstants.SYS_SERVICE_DENOMINATION; log.info("[AmbulatoryEncounterMapper] [toR5] serviceType.coding.system : {} , valeur", safeToString(svCoding.system));
            ...
            // extensions (R5)
            out.extension = EncounterExtensionsMapper.buildEncounterR5Extensions(in); log.info("[AmbulatoryEncounterMapper] [toR5] out.extension.size : {} , valeur", out.extension == null ? 0 : out.extension.size());

            // location (from relatedService if present)
            if (in.getData() != null && in.getData().getRelatedService() != null) {
                EncounterR5.Reference locRef = new EncounterR5.Reference();
                Long id = in.getData().getRelatedService().getIdentifier(); log.info("[AmbulatoryEncounterMapper] [toR5] relatedService.identifier : {} , valeur", safeToString(id));
                String name = in.getData().getRelatedService().getName(); log.info("[AmbulatoryEncounterMapper] [toR5] relatedService.name : {} , valeur", safeToString(name));
                if (id != null) {
                    locRef.reference = "Location/" + id; log.info("[AmbulatoryEncounterMapper] [toR5] location.reference : {} , valeur", safeToString(locRef.reference));
                    locRef.display = name; log.info("[AmbulatoryEncounterMapper] [toR5] location.display : {} , valeur", safeToString(locRef.display));
                    EncounterR5.EncounterLocation el = new EncounterR5.EncounterLocation();
                    el.location = locRef;
                    if (out.actualPeriod != null) {
                        el.period = out.actualPeriod; log.info("[AmbulatoryEncounterMapper] [toR5] location.period(start) : {} , valeur", safeToString(out.actualPeriod.start));
                    }
                    out.location = List.of(el); log.info("[AmbulatoryEncounterMapper] [toR5] out.location.size : {} , valeur", 1);
                }
            }
    ...
    // removed local buildEncounterR5Extensions and buildEncounterR4Extensions methods
    ...
    private static EncounterR4.Extension childR4(String url, Integer value) {
        if (value == null) return null;
        EncounterR4.Extension e = new EncounterR4.Extension(); e.url = url; e.valueInteger = value; return e;
    }
    ...
    private static class EncounterClassTriple {
        ...
        private EncounterClassTriple(String system, String code, String display) { this.system = system; this.code = code; this.display = display; }
    }
    ...
    private static String buildPatientDisplay(String first, String last) {
        String f = first == null ? "" : first; String l = last == null ? "" : last; String v = (f + " " + l).trim(); return v.isEmpty() ? null : v;
    }

    private static String normalizeCode(String s) { if (s == null) return null; return s.trim().toUpperCase().replace(' ', '_'); }
